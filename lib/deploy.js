...
exports.deployStandaloneLambdaFunction = (resource) => {
  logger.status('Deploying Standalone Lambda Function')
...
deployStuff = (resource) => {
  return cloudwatchevents.deployRule(resource.CloudwatchEvents)
    .then(exports.deployStandaloneLambdaFunction(resource))
    .then(cloudwatchevents.putTarget(resource))
}
