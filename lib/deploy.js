...
// deployPointless
exports.deployPointless = (lambda) => {
return lambda.deployLambdaFunction(lambda).then(cloudwatch.deployEvent(lambda.CloudwatchEvent))
}

// Build Pointless Resourcess
exports.deployPointlessResources = () => {
  var endpointsFilePath = aux.getRootFolder() + '/' + SETTINGS.POINTLESS_FILENAME
  var configurationFile = JSON.parse(Fs.readFileSync(endpointsFilePath, 'utf8'))

  var promise = Promise.resolve()
  configurationFile.forEach((lambda) => {
    promise = promise.then(() => deployPointless(lambda))
  })
