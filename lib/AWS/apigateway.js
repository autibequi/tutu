...
exports.deleteResource = (resource) => {
  ...
  .catch((err) => {
    return retryRequest(err, 'deleteResource', resource)
  })
}
...
exports.createDeployment = () => {
  ...
  .catch((err) => {
    return retryRequest(err, 'createDeployment', '')
  })
}

// Retrier
let retryRequest = function(err, functionName, param) {
  return new Promise((resolve, reject) => {
    ...
    if ((err.code == 'TooManyRequestsException' ||
      ...
      setTimeout(() => resolve(Promise.resolve(exports[functionName](param))), SETTINGS.TIMEOUT)
    } else {
      reject(Promimse.reject(err))
    }
  })
}
