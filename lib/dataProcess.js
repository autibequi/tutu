  ...
  return apigateway.getResourceByPath('/')
    .then((rootResource) => {
      return configurationFile.Resources.map((resource) => {
        ...
        resource.default.VpcConfig = configurationFile.Lambda.VpcConfig
        return resource
      })
    })
}
...
exports.buildResource = (resource) => {
  ...
  resource.Methods = resource.Methods.map((methodName) => {
    var method = {
      ...
      responses: buildMethodResponses(methodName, resource),
      requestParameters: buildRequestParameters(resource),
