...
// --------------------------------------
// Create a Lambda URI
exports.buildLambdaURI = (lambda) => {
  return 'arn:aws:apigateway:' + SETTINGS.REGION +
                             ':lambda:path/2015-03-31/functions/' + lambda.FunctionArn +
                             '/invocations';
}
...
exports.buildRootResources = (unifiedLambdaURI) => {
  ...
  return apigateway.getResourceByPath('/')
    .then((rootResource) => {
      return configurationFile.Resources.map((resource) => {
        ...
        resource.default.unifiedLambdaURI = unifiedLambdaURI
        return resource
      })
    })
}
...
exports.buildSubResources = (resource) => {
    return resource.Resources.map((subResource) => {
