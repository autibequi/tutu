...
exports.buildUnifiedLambdaFunction = () => {
  return new Promise((resolve, reject) => {
    ...
    // Loop all prod dependencies and archive them
    for (key in packagejson.dependencies){
      // Bulk add node_modules folder
      console.log('Packing node modules...', key);
      lambdaPackage.bulk([{
        cwd: Path.join(rootFolderPath, 'node_modules', key),
        dest: Path.join('node_modules', key),
        src: ['**/**'],
        expand: true
      }]);
    }
