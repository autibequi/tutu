...
exports.buildStandaloneLambdaFunction = (resource) => {
  return new Promise((resolve, reject) => {
    ...
    var distFolderPath = Path.join(rootFolderPath,
                                   SETTINGS.TEMP_FOLDER_NAME)
    var packageName = SETTINGS.STAGE_NAME + '_' + resource.Path.replace(/{|}|\//g, '')
    var packagePath = Path.join(distFolderPath,
                                SETTINGS.STAGE_NAME + '_' +
                                packageName + '.zip')
    ...
    lambdaPackage.bulk([{
      cwd: Path.join(rootFolderPath, SETTINGS.SOURCE_FOLDER_NAME, 'node_modules'),
      dest: Path.join(SETTINGS.SOURCE_FOLDER_NAME, 'node_modules'),
      src: ['**/**'],
      expand: true
    }]);
